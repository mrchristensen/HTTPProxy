    ...
    def run(self):
        ...
        for num, (classes, pts_for_group) in enumerate(self.test_classes):
            ...
            for cls in classes:
                ...
                print('    %s: %d/%d%s' % (p.DESCRIPTION, p.successes, p.attempts, mem_msg))
            tot_pts = pts_for_group * (successes/attempts)
            pts += tot_pts
            print('    Total: %d/%d' % (tot_pts, pts_for_group))
...
class ProxyTest:
    ...
    def cleanup_files(self):
        subprocess.call(['rm', '-r', '-f', self.proxy_dir, self.noproxy_dir, self.valgrind_log_file], stdout=subprocess.DEVNULL, stderr=subprocess.STDOUT)
    ...
    def _download_proxy(self, url, output, timeout):
        ...
        cmd = ['curl', '--max-time', str(timeout), '--silent', '--proxy', self.proxy_url, '--output', output, url]
        self.logger.debug(' '.join(cmd))
        return subprocess.Popen(cmd)

    def _download_proxy_slow(self, url, output, timeout, sleep_between_send=1):
        self.logger.info('Fetching %s into %s - through proxy' % (url, output))
        cmd = [SLOW_CLIENT, '--max-time', str(timeout), '--sleep-between-send', str(sleep_between_send), '--proxy', self.proxy_url, '--output', output, url]
        self.logger.debug(' '.join(cmd))
        return subprocess.Popen(cmd)
    ...
    def download_noproxy(self, url, output, timeout):
        ...
        cmd = ['curl', '--max-time', str(timeout), '--silent', '--output', output, url]
        self.logger.debug(' '.join(cmd))
        return subprocess.Popen(cmd)

    def _are_same(self, file1, file2):
        self.logger.debug('Comparing %s and %s' % (file1, file2))
        try:
            ...
            self.logger.debug('Files %s and %s are the same' % (file1, file2))
            return True

    def _is_newer_than(self, file1, file2):
        self.logger.debug('Comparing timestamps of %s and %s' % (file1, file2))
        try:
            if os.path.getmtime(file1) > os.path.getmtime(file2):
                self.logger.debug('%s is newer than %s' % (file1, file2))
                return True
            else:
                self.logger.error('%s is newer than %s' % (file2, file1))
                return False
        except OSError as e:
...
class BasicProxyTest(ProxyTest):
    FILES = ['home.html', 'csapp.c', 'tiny.c', 'godzilla.jpg',
                    'cgi-bin/slow?sleep=1&size=4096', 'tiny']
    ...
    def run(self):
        ...
        for (i, filename) in enumerate(self.FILES):
            dst_path_proxy = os.path.join(self.proxy_dir, '%s-%d' % (self._filesystem_safe(filename), i))
            dst_path_noproxy = os.path.join(self.noproxy_dir, '%s-%d' % (self._filesystem_safe(filename), i))
...
class ExtendedBasicProxyTest(BasicProxyTest):
    FILES = ['cgi-bin/slow?sleep=1&size=4096']
    DESCRIPTION = 'Extended Basic Proxy Test'
    ...
    def run(self):
        ...
        for (i, filename) in enumerate(self.FILES):
            ...
            try:
                ...
                self.logger.warning('Error removing downloaded file "%s": %s' % (dst_path_proxy, str(e)))
...
class ExtendedConcurrencyProxyTest(ProxyTest):
    ...
    def run(self):
        ...
        i = 0
        for i in range(i, 10):
            dst_path_proxy = os.path.join(self.proxy_dir, '%s-%d' % (self._filesystem_safe(self.SLOW_FILE), i))
            dst_path_noproxy = os.path.join(self.noproxy_dir, '%s-%d' % (self._filesystem_safe(self.SLOW_FILE), i))
            url = 'http://%s:%d/%s&i=%d' % (self.tiny_host, self.tiny_port, self.SLOW_FILE, i)

            proxy_proc = self.download_proxy_slow(url, dst_path_proxy, 10)
            noproxy_proc = self.download_noproxy(url, dst_path_noproxy, 10)
        ...
        for i in range(10, 20):
            dst_path_proxy = os.path.join(self.proxy_dir, '%s-%d' % (self._filesystem_safe(self.FAST_FILE), i))
            dst_path_noproxy = os.path.join(self.noproxy_dir, '%s-%d' % (self._filesystem_safe(self.FAST_FILE), i))
            url = 'http://%s:%d/%s?i=%d' % (self.tiny_host, self.tiny_port, self.FAST_FILE, i)
        ...
        for (proxy_proc, noproxy_proc, dst_path_proxy, dst_path_noproxy) in tried + tried_slow:
            proxy_proc.wait()
            noproxy_proc.wait()
...
def main():
    ...
    if args.check_basic is not None:
        classes.append(((BasicProxyTest, ExtendedBasicProxyTest), args.check_basic))
    if args.check_cache is not None:
        classes.append(((CacheTest, ), args.check_cache))
    if args.check_concurrency is not None:
        classes.append(((BasicConcurrencyProxyTest, ExtendedConcurrencyProxyTest), args.check_concurrency))
