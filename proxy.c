...
void *thread(void *g)
{
    ...
    sem_wait(&s_buffer.current_items);

    sem_wait(&s_buffer.mutex_lock);

    s_buffer.first_index = s_buffer.first_index + 1;

    c_file_desc = s_buffer.buffer[(s_buffer.first_index) % (s_buffer.max_num_of_items)];
    sem_post(&s_buffer.mutex_lock);
    sem_post(&s_buffer.buffered_items);
